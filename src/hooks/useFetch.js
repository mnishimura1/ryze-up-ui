...
export const useFetchMarkets = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=base-ecosystem&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=1h');
                const data = await response.json();
                const cards = data.map((coin) => ({
                ...
                updateMarkets(cards.reduce((acc, card) => ({ ...acc, [card.sym]: card }), {}));
            }
            catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const balances = { USDC: 10000, ETH: 2.5 };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const imbalance = mockTrades.reduce((acc, trade) => acc + (trade.side === 'BUY' ? 1 : -1), 0) / mockTrades.length;
                useStore.setState((state) => ({
                    flow: {
                        ...state.flow,
                        heat: {
